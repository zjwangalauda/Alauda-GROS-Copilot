...
from candidate_manager import CandidateManager, PIPELINE_STAGES, STAGE_COLORS, TERMINAL_STAGES
...
if "m7_stage_filter" not in st.session_state:
    st.session_state["m7_stage_filter"] = None

_m7cols = st.columns(len(PIPELINE_STAGES))
for _i, _stage in enumerate(PIPELINE_STAGES):
    with _m7cols[_i]:
        _color = STAGE_COLORS[_stage]
        _is_selected = st.session_state["m7_stage_filter"] == _stage
        _border_width = "3px" if _is_selected else "1px"
        _border_color = _color if _is_selected else "#E2E8F0"
        _bg = "#F0F7FF" if _is_selected else "#fff"
        if st.button(
            f"{stats[_stage]}\n{_stage}",
            key=f"stat_{_stage}",
            use_container_width=True,
        ):
            if st.session_state["m7_stage_filter"] == _stage:
                st.session_state["m7_stage_filter"] = None  # Toggle off
            else:
                st.session_state["m7_stage_filter"] = _stage
            st.rerun()

# Show active filter indicator
_stage_filter = st.session_state["m7_stage_filter"]
if _stage_filter:
    _fc = STAGE_COLORS[_stage_filter]
    st.markdown(
        f"<div style='background:{_fc}15;border:1px solid {_fc};border-radius:6px;padding:8px 12px;"
        f"margin:8px 0;display:flex;justify-content:space-between;align-items:center;'>"
        f"<span style='color:{_fc};font-weight:600;'>Viewing: {_stage_filter} ({stats[_stage_filter]})</span>"
        f"</div>",
        unsafe_allow_html=True,
    )
    _filter_btn_col1, _filter_btn_col2 = st.columns([6, 1])
    with _filter_btn_col2:
        if st.button(bi("Clear filter", "清除筛选"), use_container_width=True):
            st.session_state["m7_stage_filter"] = None
            st.rerun()
...
with st.expander(bi("➕ Add Candidate", "➕ 添加新候选人"), expanded=(_total == 0)):
    ...
    with st.form("add_candidate_form", clear_on_submit=True):
        ...
        with _c1:
            ...
            _cand_source = st.selectbox(bi("Source Channel", "来源渠道"), ["LinkedIn X-Ray", "GitHub", "Referral", "Job Board", "Auto Sourcing", "Other"])
        with _c2:
...
if _stage_filter:
    # Filtered view: show only the selected stage
    _display_stages = [_stage_filter]
else:
...
for _col_idx, _stage in enumerate(_display_stages):
    with _kanban_cols[_col_idx]:
        ...
        for _cand in _stage_candidates:
            ...
            _next_stages = [s for s in PIPELINE_STAGES if s != _stage and s not in TERMINAL_STAGES]
            ...
            if _stage not in TERMINAL_STAGES:
                ...
                # For terminal stages: allow re-activation with note
                if st.button(bi("Re-activate", "重新激活"), key=f"react_{_cand['id']}", use_container_width=True):
                    if _move_note.strip():
                        try:
                            cm.move_stage(_cand["id"], _move_to, note=_move_note)
                            st.rerun()
                        ...
                        st.error(bi("A note is required to re-activate.", "重新激活需要填写备注说明原因。"))
...
if not _all_cands:
    ...
    # If a stage filter is active, pre-filter the dropdown
    if _stage_filter:
        _filtered_cands = [c for c in _all_cands if c["stage"] == _stage_filter]
    else:
        _filtered_cands = _all_cands

    if not _filtered_cands:
        st.info(bi(f"No candidates in {_stage_filter} stage.", f"{_stage_filter} 阶段暂无候选人。"))
    else:
        _cand_names = {f"{c['name']} ({c['stage']})": c["id"] for c in _filtered_cands}
        ...
        if _selected:
            # Stage badge
            _sel_color = STAGE_COLORS.get(_selected["stage"], "#64748B")
            st.markdown(
                f"<span style='background:{_sel_color};color:#fff;padding:4px 12px;border-radius:12px;"
                f"font-size:0.85rem;font-weight:600;'>{_selected['stage']}</span>",
                unsafe_allow_html=True,
            )
