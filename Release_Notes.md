...
- **密码比较安全升级 (P2)**：登录密码验证从 `==` 直接比较改为 `hmac.compare_digest()`，消除时序攻击 (Timing Attack) 风险。

### 📦 依赖补全 (Dependencies)
- **缺失依赖声明修复 (P2)**：`requirements.txt` 补全 `pandas==2.3.3` 和 `langchain-text-splitters==1.1.1`，修复全新环境部署时 `ModuleNotFoundError` 的问题。

### 🧪 测试覆盖率跃升 (Testing)
- **测试用例从 39 → 78**：新增 39 个单元测试，覆盖 `document_parser.py`、`recruitment_agent.py`、`app_shared.py`。

### 🏗️ 工程化改进 (Infrastructure)
- **CI/CD 增强**：GitHub Actions 新增 `pip` 缓存与 `pytest-cov` 覆盖率报告（最低阈值 30%）。

---

## V2.0.1 — 热修复：LLM 引擎凭证缓存失效
**Release Date:** 2026-03-02

### 🔧 修复 (Bug Fixes)
- **LLM 凭证缓存失效**：修复当用户在 Streamlit Secrets 中更新 API Key 后，系统仍使用旧凭证进行 LLM 调用的问题。现在每次检测到凭证变更时，自动重新初始化 Agent 实例，确保配置变更即时生效。
